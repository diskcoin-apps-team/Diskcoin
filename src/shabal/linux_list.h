...
/**
 * list_add_first - add a new1 entry
 ...
 */
static inline void list_add_first(struct list_head *new1, struct list_head *head)
{
...
#define list_next_entry(pos, member) \
	list_get_entry((pos)->member.next, typeof(*(pos)), member)
...
#define list_prev_entry(pos, member) \
	list_get_entry((pos)->member.prev, typeof(*(pos)), member)
...
#define list_prepare_entry(pos, head, member) \
	((pos) ? : list_get_entry(head, typeof(*pos), member))
...
static inline unsigned hlist_str_hash (const char *str, unsigned n_bucket) {
	unsigned h = 0;
	if (str) {
		while (*str) {
...
{
	__hlist_del(n);
	n->next = NULL;//(struct hlist_node *)LIST_POISON1;
	n->pprev = NULL;//(struct hlist_node **)LIST_POISON2;
}
