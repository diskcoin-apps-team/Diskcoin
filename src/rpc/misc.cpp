...
UniValue validateaddress(const UniValue &params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            ...
            "\nReturn information about the given diskcoin address.\n"
            "\nArguments:\n"
            "1. \"diskcoinaddress\"     (string, required) The diskcoin address to validate\n"
            "\nResult:\n"
            "{\n"
            "  \"isvalid\" : true|false,       (boolean) If the address is valid or not. If not, this is the only "
            "property returned.\n"
            "  \"address\" : \"diskcoinaddress\", (string) The diskcoin address validated\n"
...
#ifdef ENABLE_WALLET
        // Case 1: Diskcoin address and we have full public key:
...
UniValue createmultisig(const UniValue &params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 2)
    {
        string msg =
            ...
            "2. \"keys\"       (string, required) A json array of keys which are diskcoin addresses or hex-encoded "
            "public keys\n"
            "     [\n"
            "       \"key\"    (string) diskcoin address or hex-encoded public key\n"
            "       ,...\n"
            "     ]\n"
...
UniValue verifymessage(const UniValue &params, bool fHelp)
{
    if (fHelp || params.size() != 3)
        throw runtime_error(
            ...
            "\nArguments:\n"
            "1. \"diskcoinaddress\"  (string, required) The diskcoin address to use for the signature.\n"
